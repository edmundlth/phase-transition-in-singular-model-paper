...
def expected_nll(X, Y, param_dict, sigma_obs):
    ...
    ydist = dist.Normal(z3, sigma_obs)
...
def model(X, Y, D_H, itemp, D_Y=1):
    ...
    with numpyro.plate("data", N):
        ...
        numpyro.sample("Y", dist.Normal(z3, sigma_obs/np.sqrt(itemp)).to_event(1), obs=Y)


# helper function for HMC inference
def run_inference(model, args, rng_key, X, Y, D_H, itemp):
    start = time.time()
    kernel = NUTS(model)
    mcmc = MCMC(
        kernel,
...
def main(args):
    ...
    X, Y, X_test = get_data(N=N, D_X=D_X, sigma_obs=args.sigma_obs)
    ...
    ax.plot(1 / itemps, 1 / itemps * slope + intercept,
            label=f"$\lambda$={slope:.3f}, fitted $nL_n(w_0)$={intercept:.3f}, R^2={r_val ** 2}")
