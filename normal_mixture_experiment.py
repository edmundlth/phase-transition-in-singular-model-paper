...
def build_gaussian_mixture_model(
    component_dim, num_component=2, sigma=1.0, prior_sigma=10.0, alpha=0.3
):
    def gaussian_mixture_model(data, itemp=1.0):
        weights = numpyro.sample(
            "weights", dist.Dirichlet(concentration=np.ones(num_component) * alpha)
        )
        mus = [
            numpyro.sample(
                f"mu{i + 1}",
            ...
            numpyro.sample(
                ...
                dist.MixtureGeneral(
                    ...
                    component_distributions=[
                        dist.MultivariateNormal(
                            loc=mu, covariance_matrix=jnp.eye(component_dim) * sigma
                        )
                        for mu in mus
                    ],
                ),
...
def main(args, rngseed):
    ...
    loglike_fn = functools.partial(
        log_likelihood_func, component_dim=args.component_dim, sigma=args.sigma
    )
    ...
    mcmc = run_mcmc(
        model, data, rngkey, mcmc_config, itemp=1.0, progress_bar=(not args.quiet)
    )
    ...
    posterior_param_list = [
        {key: posterior_samples[key][i] for key in posterior_samples.keys()}
        for i in range(num_mcmc_samples)
    ]

    loglike_array = _compute_loglike_array(loglike_fn, data, posterior_param_list)
    loglike_array_test = _compute_loglike_array(
        loglike_fn, test_data, posterior_param_list
    )
    ...
    if args.outdirpath:
        outfilepath = os.path.join(
            args.outdirpath, f"result_{args.num_training_data}_{rngseed}.json"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=2,
        type=int,
        help="Dimension of the component Gaussian model.",
    )
    parser.add_argument(
        "--mixture_weights",
        nargs="+",
        default=[0.5, 0.5],
        type=float,
        help="Mixture weights. Will be normalised before use so that they sum to unity.",
    )
    ...
    parser.add_argument(
        ...
        default=0.3,
        type=float,
        help="Parameter for Dirichlet distribution as prior for mixture weights.",
    )
    ...
    if args.outdirpath:
        logger.info(
            f"Result to be saved at directory: {os.path.abspath(args.outdirpath)}"
        )
        os.makedirs(args.outdirpath, exist_ok=True)

    for i, rngseed in enumerate(args.rng_seeds):
        logger.info(
            f"Starting experiment {i} of {len(args.rng_seeds)} with rngseed: {rngseed}"
        )
        main(args, rngseed)
        logger.info(
            f"Finished experiment {i} of {len(args.rng_seeds)} with rngseed: {rngseed}"
        )
